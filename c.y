...
int indirectTripleCodeGenerator(GrammarTree node, struct Instruction* instruction) {
    ...
    switch (node->opt) {
        ...
        case _FOR:
            ...
            node->end = makeNewTemp(instruction, generateIndirectTriple("j", "_", "_")); // break
...
//            }
            ...
            rewriteTemp(instruction, node->child[0]->child[2]->end, 2, go); // action -> jump to condition head.
            // Rewrite stmt head.
            // Rewrite stmt tail.
            // Rewrite action head.
            // End.
...
//            }
        ...
        case _DO_WHILE:
            pushScope(0);
            indirectTripleCodeGenerator(node->child[0], instruction); // DO
            popScope();
            indirectTripleCodeGenerator(node->child[1], instruction); // WHILE_CONDITION
            node->begin = node->child[0]->begin;
            node->head = node->child[0]->head;
            ...
            break;
        case _DO:
            indirectTripleCodeGenerator(node->child[0], instruction); // LOOP
            node->begin = node->child[0]->begin;
            node->end = node->child[0]->end;
            break;
        case _WHILE:
